// =====================================================
// ADICIONAR PROCESSO: Preparação de Revel (Costura)
// =====================================================
...
async function adicionarProcesso() {
  try {
    console.log('🚀 Adicionando processo "Preparação de Revel" ao módulo Costura...\n');
    ...
    const { data: processoExistente } = await supabaseAdmin
      ...
      .eq('nome', 'Preparação de Revel')
      .single();

    if (processoExistente) {
      console.log(`⚠️  Processo "Preparação de Revel" já existe (ID: ${processoExistente.id})`);
