-- ==========================================================
-- MIGRATION 002: Adicionar Módulo de Estampas (SAFE VERSION)
-- Data: 2025-11-03
-- Descrição: Adiciona tabelas e campos para Bordado, DTF e Patch
-- Impacto: ZERO no módulo Acabamento
-- SAFE: Remove UPDATE que depende de nomes de colunas específicos
-- ==========================================================
...
-- ===========================
-- 7. MODIFICAR: users
-- ⚠️ MUDANÇA: Não faz UPDATE automático, você pode fazer manualmente depois
-- ===========================
ALTER TABLE users ADD COLUMN IF NOT EXISTS modulos_permitidos TEXT[] DEFAULT ARRAY['acabamento'];

-- ⚠️ ATENÇÃO: Após a migration, execute manualmente via Supabase Dashboard:
-- UPDATE users SET modulos_permitidos = ARRAY['acabamento', 'estampas']
-- WHERE <sua_coluna_de_papel> = 'admin' OR <sua_coluna_de_papel> = 'gestor';

RAISE NOTICE '';
RAISE NOTICE '⚠️  AÇÃO MANUAL NECESSÁRIA:';
RAISE NOTICE 'Após a migration, atualize os usuários admin/gestor manualmente:';
RAISE NOTICE '';
RAISE NOTICE 'UPDATE users SET modulos_permitidos = ARRAY[''acabamento'', ''estampas'']';
RAISE NOTICE 'WHERE <coluna_papel> IN (''admin'', ''gestor'');';
RAISE NOTICE '';
...
BEGIN
  ...
  RAISE NOTICE '1. Execute validate_migration.sql';
  RAISE NOTICE '2. Atualize usuários admin manualmente (ver mensagem acima)';
  RAISE NOTICE '';
END $$;
